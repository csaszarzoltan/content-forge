...
function App(){
 ...
 async function requestReview(){
  ...
  try{await request(`/api/v1/assets/${active.id}/approval`,{method:"POST",body:JSON.stringify({requester:"You",risk:"MEDIUM",findings:[]})}); setSaveState(`Version ${active.version} sent for review`); if(cockpit)await refresh(cockpit.campaign.id);}
  catch(reason){setError(validationMessage(reason));}
 }
